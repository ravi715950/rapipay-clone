...
export default function Services({ current }) {
  const [activeCard, setActiveCard] = useState(serviceData[0])
  ...
  useEffect(() => {
    var item = serviceData.filter((item) => { return '#' + item.id == current })
    if (item.length > 0) {
    ...
    <Box sx={{ marginTop: '132px', marginBottom: '180px', backgroundImage: 'url("backgroundscroll.webp")', backgroundRepeat: 'no-repeat', backgroundSize: 'cover', paddingLeft:{xs:'none', md:'45px'}, marginLeft: {xs:'none', md:'15px'}, padding:{xs:'15px', md:'none'}  }} >
      <Grid sx={{ display: { xs: 'none', md: 'flex' } }} container spacing={2} >
          ...
          <Grid container spacing={2}>
            {serviceData.map((item, index) => (
              <Grid id={item.id} onClick={() => handleSetActiveCard(item)} key={index} item xs={12} md={6} style={{ marginTop: item.marginTop }}>
                <Box className={activeCard.cardname === item.cardname ? "active" : "serviceCardBox"}>
        ...
        <Grid item xs={12} md={5} >
          <Box sx={{ position: 'sticky', top: '200px', color: 'var(--main-color)', width: '30vw' }}>
            ...
            <Box key={index}  sx={{
              ...
              <Box 
               sx={{
                ...
                />
                <Typography sx={{ fontSize: '16px', fontFamily: 'Poppins', fontWeight: '600', marginTop: '40px', paddingBottom: '16px', color: 'var(--main-color)' }} >{item.heading}</Typography>
              </Box>
              <Box sx={{ marginTop: '32px', paddingBottom:'30px' }}>
                <Typography sx={{ marginBottom: '25px', fontSize: '25px', color: 'var(--main-color)', fontFamily: 'Poppins', fontWeight: '700' }} >{item.cardname}</Typography>
                {item.discription.map((text, index) => (<Box display={'flex'}>
                  <CheckCircleIcon sx={{ marginRight: '14px', color: 'var(--main-color)', paddingTop: '10px' }} />
                  <Typography key={index} sx={{ fontSize: '14px', fontFamily: 'Poppins', fontWeight: '500', color: 'var(--main-color)', paddingTop: '10px' }}  >{text}</Typography>
                </Box>
                ))}
              </Box>
